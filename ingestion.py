...
# chroma = Chrome(persist_directory="chroma_db", embedding_function=embeddings)
...
async def index_documents_async(documents: List[Document], batch_size: int = 50):
    """Process documents inbatches asynchronously."""
...
async def main():
    ...
    res = tavily_crawl.invoke(
        {
            ...
            "max_depth": 2,
            "extract_depth": "advanced",  # gets more content from the page, not just the main body
        }
    )

    all_docs = [
        Document(page_content=result["raw_content"], metadata={"source": result["url"]}) for result in res["results"]
    ]
    ...
    log_info(
        f" Text splitter: Processing {len(all_docs)} documents with 4000 character chunks and 200 character overlap",
        Colors.YELLOW,
    )
